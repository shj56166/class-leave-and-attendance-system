...
import { StudentDeviceSettings } from '../services/nativeDeviceSettings';

const STORAGE_KEY = 'student_server_runtime';
...
export function normalizeServerOrigin(input, options = {}) {
...
function normalizeApiBaseUrl(input, options = {}) {
  const trimmed = String(input || '').trim();
  if (!trimmed) {
    return '';
  }
  ...
  if (trimmed.startsWith('/')) {
    return stripTrailingSlash(trimmed) || DEFAULT_API_PATH;
  }

  const defaultProtocol = options.defaultProtocol || 'http';
  const allowHttp = options.allowHttp !== false;
  const allowHttps = options.allowHttps !== false;
  const defaultPath = stripTrailingSlash(options.defaultPath || DEFAULT_API_PATH) || DEFAULT_API_PATH;
  const withProtocol = hasExplicitProtocol(trimmed)
    ? trimmed
    : `${defaultProtocol}://${trimmed}`;
  ...
  try {
    ...
    url.pathname = stripTrailingSlash(url.pathname && url.pathname !== '/' ? url.pathname : defaultPath) || defaultPath;
    url.search = '';
    url.hash = '';
...
function buildDerivedApiBaseUrl(serverOrigin) {
  if (serverOrigin) {
    return `${serverOrigin}${DEFAULT_API_PATH}`;
  }
...
function resolveWebApiBaseUrl() {
  const explicitSecureApiBaseUrl = normalizeApiBaseUrl(getEnvValue('VITE_WEB_SECURE_API_BASE_URL'), {
    defaultProtocol: 'https',
    allowHttp: false
  });
  if (explicitSecureApiBaseUrl) {
    return explicitSecureApiBaseUrl;
  }
  ...
  if (explicitSecureOrigin) {
    return `${explicitSecureOrigin}${DEFAULT_API_PATH}`;
  }

  const legacySecureApiBaseUrl = normalizeApiBaseUrl(getEnvValue('VITE_API_BASE_URL'), {
    defaultProtocol: 'https',
    allowHttp: false
  });
...
function resolveAndroidDebugOrigin() {
  return extractOriginFromUrl(getEnvValue('VITE_ANDROID_NATIVE_DEBUG_API_BASE_URL'))
...
function applyServerOrigin(origin, source = 'manual') {
  ...
  state.apiBaseUrl = buildDerivedApiBaseUrl(normalizedOrigin);
  state.source = source;
...
function markFailure(message) {
  ...
  state.lastError = String(message || '未找到可用服务器').trim();
}
...
function seedInitialState() {
  ...
  if (isWebRuntime()) {
    const apiBaseUrl = buildDerivedApiBaseUrl('');
    state.serverOrigin = extractOriginFromUrl(apiBaseUrl, {
      defaultProtocol: 'https',
      allowHttp: false
  ...
  state.apiBaseUrl = buildDerivedApiBaseUrl(preferredOrigin);
  state.lastVerifiedAt = saved.lastVerifiedAt;
  state.lastSuccessOrigin = saved.lastSuccessOrigin;
...
async function readNetworkInfo() {
  ...
  return StudentDeviceSettings.getNetworkInfo().catch(() => ({
    ipv4Address: '',
    gatewayAddress: '',
    subnetMask: '',
    subnetPrefix: ''
...
export async function discoverServerOrigin() {
  ...
  discoveryPromise = (async () => {
    ...
    if (!candidates.length) {
      markFailure('未拿到可用的局域网地址');
      return '';
    }

    const foundOrigin = await verifyCandidates(candidates);
    if (!foundOrigin) {
      markFailure('未自动发现可用服务器，请手动填写地址');
      return '';
    }
...
export function resolveApiBaseUrl() {
  return state.apiBaseUrl || (isWebRuntime() ? '/api' : '');
}
